...
def create_extraction_test_bill(path: Path) -> None:
    """Create a synthetic electricity bill for extraction testing."""

    pdf = canvas.Canvas(str(path))

    pdf.drawString(72, 760, "GRIDPILOT EXTRACTION TEST BILL")
...
def test_extractor_finds_expected_fields(tmp_path: Path):
...
def test_extractor_normalizes_values(tmp_path: Path):
    pdf_path = tmp_path / "bill.pdf"
    create_extraction_test_bill(pdf_path)
    ...
    assert fields["consumption_kwh"].normalized_value == 92482.0
    assert fields["maximum_demand_kva"].normalized_value == 417.0
    assert fields["power_factor"].normalized_value == 0.89
    assert fields["power_factor_penalty"].normalized_value == 24000.0
    assert fields["total_cost"].normalized_value == 524381.0


def test_extractor_preserves_evidence(tmp_path: Path):
    pdf_path = tmp_path / "bill.pdf"
    create_extraction_test_bill(pdf_path)
    ...
    evidence = fields["power_factor"].evidence

    assert evidence is not None
    assert evidence.page == 1
    assert evidence.raw_text == "Average Power Factor: 0.89"
    assert evidence.source_label == "Average Power Factor"
    assert evidence.confidence == 1.0


def test_complete_bill_does_not_require_review(tmp_path: Path):
    pdf_path = tmp_path / "bill.pdf"
    create_extraction_test_bill(pdf_path)
...
def test_missing_required_field_requires_review(tmp_path: Path):
    pdf_path = tmp_path / "incomplete_bill.pdf"

    pdf = canvas.Canvas(str(pdf_path))
    pdf.drawString(72, 760, "Units Consumed: 50,000 kWh")
    pdf.save()
